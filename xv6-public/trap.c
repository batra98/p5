...
void trap(struct trapframe *tf) {
  ...
  case T_PGFLT: {
    ...
    if (pte != 0) {
      if (!(*pte & PTE_COW)) {
        cprintf("pte Segmentation Fault\n");
        kill(p->pid);
        exit();
      }
      ...
      perform_mapping(p->pgdir, (char *)fault_addr, PGSIZE, V2P(new_page),
                      PTE_W | PTE_U);
      decrement_ref_count(pa);
      lcr3(V2P(p->pgdir));

    } else {
      cprintf("in no pte zone\n");

      int mapped = 0;

      for (int i = 0; i < p->mmap_count; i++) {
        struct mmap_region *region = &p->mmap_regions[i];
        if (fault_addr >= region->addr &&
          ...
          int result = perform_mapping(p->pgdir, (void *)fault_addr, PGSIZE,
                                       V2P(mem), PTE_W | PTE_U);
          if (result != 0) {
      ...
      if (!mapped) {
        cprintf("no pte Segmentation Fault\n");
        kill(p->pid);
      }
    }
